...
static		uint8_t		*content_begin;
static		uint32_t	buf_len = 5242880;	/* 5MB */
#endif
...
r_status _log_files_circular_insert(const char * filename, struct logger *l)
{
	...
	if(l->circular_cur_pos < 0)
	{
		...
		l->log_files_circular[l->circular_cur_pos] = rct_zalloc(strlen(filename) + 1);
		strcpy(l->log_files_circular[l->circular_cur_pos], filename);
	}
	else if(l->circular_cur_pos == max_array_id)
	{
		for(i = max_array_id; i >=0; i --)
		{
			...
			else if(cmpresult < 0 && i == 0)
			{
				...
				l->log_files_circular[i] = rct_zalloc(strlen(filename) + 1);
			...
			else if(i < max_array_id)
			{
				...
				l->log_files_circular[i + 1] = rct_zalloc(strlen(filename) + 1);
		...
		{
			rct_free(l->log_files_circular[max_array_id+1]);
...
r_status log_files_circular_init(struct logger *l)
{
	...
	//l->log_files_circular[l->circular_cur_pos + 1] = rct_zalloc(strlen(logname) + 1);
...
r_status _log_files_circular_deinit(struct logger *l)
{
	...
	for(i = 0; i < LOG_FILES_CIRCULAR_MAX_LEN; i ++)
	{
		...
		{
			rct_free(l->log_files_circular[i]);
		}
	}
	l->circular_full = false;
	l->circular_cur_pos = -1;
...
set_log_file_max_size(char *arg)
{
    ...
    ssize_t num;
	char *pos;
	char *result = rct_zalloc(120*sizeof(char));
    char *value;
	int value_len;
	size_t i;
...
receive_buf_init()
{
	receive_buf = rct_zalloc(buf_len);
	if(receive_buf == NULL)
	{
		return RCT_ENOMEM;
	}
